def tallenna_summat(kvartaalitulokset, tiedosto):
    try:
        with open(tiedosto, "w") as kohde:
            for kvartaalit in kvartaalitulokset:
                i = 0
                while i < len(kvartaalit):
                    kohde.write(f"{str(kvartaalit[i])}")
                    if i < (len(kvartaalit)-1):
                        kohde.write(":")
                    i += 1
                kohde.write("\n")
    except FileNotFoundError: 
        print("Kohdetiedostoa ei ole. Tallennus epäonnistui. Tyhmyri.")
